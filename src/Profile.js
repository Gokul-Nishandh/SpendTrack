...
const Profile = ({ userName,onLogout }) => {
  ...
  return (
    ...
    <div className="profilebar">
      <Link to="/profile/view-profile"><div><CgProfile /><p>{userName}<br />View/Edit Profile</p>
      </div></Link>
      <div><MdOutlineSwitchAccount /><p>Switch Account</p></div>
      <div><IoIosSettings />Settings
      </div>
      <div onClick={handleLogout} style={{ cursor: 'pointer' }}><IoIosLogOut /><p>Log Out</p></div>
      <Link to="/"><div>Close profile</div></Link>
    </div>
    </main>
  )
}
